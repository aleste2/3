...
func (_ *HeunAFLLBPRB054401) Step() {
	...
	if err < MaxErr || Dt_si <= MinDt || FixDt != 0 { // mindt check to avoid infinite loop
		...
		// Renormalization
		if RenormLLB == true {
			RenormAF(y01, y02, float32(Dt_si), float32(GammaLL1), float32(GammaLL2))
		}
		NSteps++
...
func torqueFnAFLLBPRB(dst1, dst2 *data.Slice, hth1a, hth2a, hth1b, hth2b *data.Slice) {
	...
	if Precess {
		if MFA == false {
			cuda.LLBTorqueAFPRB(dst1, M1.Buffer(), dst2, M2.Buffer(), dst1, dst2, temp, alpha, alpha1, alpha2, Tcurie, Msat, Msat1, Msat2, hth1a, hth2a, hth1b, hth2b, X_TM, NV, MU1, MU2, J0AA, J0BB, J0AB, Lambda0) // overwrite dst with torque
		} else {
			cuda.LLBTorqueAFMFA(dst1, M1.Buffer(), dst2, M2.Buffer(), dst1, dst2, temp, alpha, alpha1, alpha2, Tcurie, Msat, Msat1, Msat2, hth1a, hth2a, hth1b, hth2b, X_TM, NV, MU1, MU2, J0AA, J0BB, J0AB, Lambda0) // overwrite dst with torque
		}
	} else {
