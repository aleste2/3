...
// FCTS 2T method, can be used as solver.Step
type Only2T struct {
}
...
func AdaptativeFTCSStep2T(dt float32) {
	...
	Substeps := 1
	cuda.Evaldt02T(te, DeltaTe, tl, DeltaTl, y, kel.Gpu(), Cel, kll.Gpu(), Clat, Gellat,
	...
	for i := 0; i < int(nsteps); i++ {
		...
		cuda.Madd2(tl, tl, DeltaTl, 1, float32(dtT)) // temp = temp + dt * dtemp0
		Time = Time + dtT
	}
	...
	cuda.Evaldt02Ttausubs(tl, DeltaTl, Tsubsth, Tausubsth, M.Mesh(), geometry.Gpu())
	err = cuda.MaxAbs(DeltaTl)
	if float64(err)*(Time-Lasttsubs) > 0.00005 {
		cuda.Madd2(tl, tl, DeltaTl, 1, float32((Time - Lasttsubs))) // temp = temp + dt * dtemp0
		Lasttsubs = Time
	}
