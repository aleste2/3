...
__global__ void
LLBtorqueAF2TMFA2(float* __restrict__  t1x, float* __restrict__  t1y, float* __restrict__  t1z,
  ...
  float* __restrict__  lambda0_, float lambda0_mul,
    ...
    if (i < N) {
		...
		float alphabe=alphaa*2*dL(mchiA)/mchiA;
		...
		float h_perp_scalea=0*sqrt(Msat/Msata*(alphaperpA-alphaparA)/(alpha*alphaperpA*alphaperpA));
		float h_perp_scaleb=0*sqrt(Msat/Msatb*(alphaperpB-alphaparB)/(alpha*alphaperpB*alphaperpB));
