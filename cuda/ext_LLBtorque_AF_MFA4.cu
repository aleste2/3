...
__global__ void
LLBtorqueAFMFA4(float* __restrict__  t1x, float* __restrict__  t1y, float* __restrict__  t1z,
    ...
    if (i < N) {
        ...
        if ((ma==0)||(mb==0))	{
         	...
         	torqueb = 0.0f*m2;
					...
					float GammaAperp;
					float GammaBperp;
						m0A=NL(mchiA)/mchiA*chiA;
						m0B=NL(mchiB)/mchiB*chiB;
						lambdaA=2.0f*alphaa*kB*temp/mua;
						lambdaB=2.0f*alphab*kB*temp/mub;
						GammaApara=lambdaA*NL(mchiA)/(mchiA*DNL(mchiA));
						GammaBpara=lambdaB*NL(mchiB)/(mchiB*DNL(mchiB));
						GammaAperp=lambdaA/2.0f*(mchiA/NL(mchiA)-1.0f);
						GammaBperp=lambdaB/2.0f*(mchiB/NL(mchiB)-1.0f);
		...
		float alphaex;
		float alphaae=alphaa*2*NL(mchiA)/mchiA;
		float alphabe=alphab*2*NL(mchiB)/mchiB;
		alphaex=0.5f*(alphaae/x/ma+alphabe/(1.0f-x)/mb);
		float3 Ha=1/(beta*mua*DNL(mchiA))*(m1-m0A);
		float3 Hb=1/(beta*mub*DNL(mchiB))*(m2-m0B);
...
*/
    ...
    float gillbb = 1.0f / (1.0f + alphab * alphab);
		...
		torquea = -gillba*(m1xHmfa+GammaApara*(1.0f-m1dotm0a/ma/ma)*m1+GammaAperp/ma/ma*(m1xm1xHtot1))+h_par_scalea*hth2a+gillba*alphaex*(mua*Ha-mub*Hb);
		torqueb = -gillbb*(m2xHmfa+GammaBpara*(1.0f-m2dotm0b/mb/mb)*m2+GammaBperp/mb/mb*(m2xm2xHtot2))+h_par_scaleb*hth2b-gillbb*alphaex*(mua*Ha-mub*Hb);
    }
    t1x[i] = torquea.x;
    t1y[i] = torquea.y;
    t1z[i] = torquea.z;
