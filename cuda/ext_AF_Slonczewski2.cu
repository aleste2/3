...
addslonczewskitorque2AF(float* __restrict__ tx, float* __restrict__ ty, float* __restrict__ tz,
	...
	if (Ms1>Ms2) {Ms=Ms1-Ms2;} else {Ms=Ms2-Ms1;}	

        if (J == 0.0f || Ms == 0.0f) {
            return;
        }
//	float m2=dot(m,m);  /// Quitar despues de probar
//        float beta    = (HBAR / QE) * (J / (flt*Ms*pow(1.0f*m2,0.5f)) );
