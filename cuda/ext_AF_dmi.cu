...
adddmiAF(float* __restrict__ Hx, float* __restrict__ Hy, float* __restrict__ Hz,
       float* __restrict__ mx, float* __restrict__ my, float* __restrict__ mz,
       float* __restrict__ m2x, float* __restrict__ m2y, float* __restrict__ m2z,
       float* __restrict__ Ms_, float Ms_mul,
       float* __restrict__ aLUT2d,
       float* __restrict__ dLUT2d,
       float* __restrict__ aLUT2d2,
       float* __restrict__ dLUT2d2,
       float* __restrict__ aLUT2d12,
    ...
    float3 m02 = make_float3(m2x[I], m2y[I], m2z[I]); // central m
    ...
    // x derivatives (along length)
    {
        ...
        float An2 = aLUT2d2[symidx(r0, r1)];                 // inter-region Aex
        float Dn2 = dLUT2d2[symidx(r0, r1)];                 // inter-region Dex
        float An12 = aLUT2d12[symidx(r0, r1)];                 // inter-region Aex
        if (!is0(m1) || !OpenBC){                          // do nothing at an open boundary
            if (is0(m1)) {                                 // neighbor missing
                m1.x = m0.x - (-cx * (0.5f/(A1*(1.0f-An12*An12/A1/An2))) *(D1*m0.z-Dn2*An12/An2*m02.z)); // extrapolate missing m from Neumann BC's
                m1.y = m0.y;
                m1.z = m0.z + (-cx * (0.5f/(A1*(1.0f-An12*An12/A1/An2))) *(D1*m0.x-Dn2*An12/A1*m02.x));
            }
...

    {
        ...
        float An2 = aLUT2d2[symidx(r0, r2)];                 // inter-region Aex
        float Dn2 = dLUT2d2[symidx(r0, r2)];                 // inter-region Dex
        float An12 = aLUT2d12[symidx(r0, r2)];                 // inter-region Aex
        if (!is0(m2) || !OpenBC){
            if (is0(m2)) {
                m2.x = m0.x - (cx * (0.5f/(A2*(1.0f-An12*An12/A2/An2))) *(D2*m0.z-Dn2*An12/An2*m02.z));
                m2.y = m0.y;
                m2.z = m0.z + (cx * (0.5f/(A2*(1.0f-An12*An12/A2/An2))) *(D2*m0.x-Dn2*An12/A2*m02.x));
            }
    ...
    // y derivatives (along height)
    {
        ...
        if (!is0(m1) || !OpenBC){
            if (is0(m1)) {
                m1.x = m0.x;
                m1.y = m0.y - (-cy * (0.5f/(A1*(1.0f-An12*An12/A1/An2))) *(D1*m0.z-Dn2*An12/A1*m02.z));
                m1.z = m0.z + (-cy * (0.5f/(A1*(1.0f-An12*An12/A1/An2))) *(D1*m0.y-Dn2*An12/A1*m02.y));
            }
...

    {
        ...
        if (!is0(m2) || !OpenBC){
            if (is0(m2)) {
                m2.x = m0.x;
                m2.y = m0.y - (cy * (0.5f/(A2*(1.0f-An12*An12/A2/An2))) *(D2*m0.z-Dn2*An12/A2*m02.z));
                m2.z = m0.z + (cy * (0.5f/(A2*(1.0f-An12*An12/A2/An2))) *(D2*m0.y-Dn2*An12/A2*m02.y));
            }
