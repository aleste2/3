...
func LLBTorqueAF2TBrillouin(torque1, m1, torque2, m2, B1, B2 *data.Slice, temp *data.Slice, alpha, alpha1, alpha2, TCurie, Msat, Msat1, Msat2 MSlice, hth1a *data.Slice, hth2a *data.Slice, hth1b *data.Slice, hth2b *data.Slice, x, nv, mua, mub, J0aa, J0bb, J0ab, lambda0, JA, JB MSlice) {
	N := torque1.Len()
	cfg := make1DConf(N)
	k_LLBtorqueAF2TBrillouin_async(torque1.DevPtr(X), torque1.DevPtr(Y), torque1.DevPtr(Z),
		...
		JA.DevPtr(0), JA.Mul(0),
		JB.DevPtr(0), JB.Mul(0),
		N, cfg)
}
