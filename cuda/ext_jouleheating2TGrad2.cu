...
extern "C"
 ...
 __global__ void
evaldt02Tgrad2(float* __restrict__  tempe_,      float* __restrict__ dt0e_,
...
//    float3 m_; // neighbor mag
    ...
    if (ix-1>=0){
      i_  = idx(ix-1, iy, iz);           // clamps or wraps index according to PBC
        ...
        if (mm_!=0)
        {
          ...
          dt0e_[i] += (Ke*(tempve-tempe)/wx/wx)*tempe/templ;
          d1 += (Ke*(tempe/templ-tempve/tempvl))/wx*(tempe-tempve)/wx;
          f1=1;
    ...
    if (ix+1<Nx){
      ...
      if (mm_!=0)
      {
        ...
        Kl = Kl_[symidx(r0, regions[i_])];
        dt0e_[i] += (Ke*(tempve-tempe)/wx/wx)*tempe/templ;
        d1 += (Ke*(tempve/tempvl-tempe/templ))/wx*(tempve-tempe)/wx;
        f1+=1;
        dt0l_[i] += (Kl*(tempvl-templ)/wx/wx);
      }
    }
    ...
    if (iy-1>=0){
      i_  = idx(ix, iy-1, iz);          // clamps or wraps index according to PBC
        ...
        if (mm_!=0)
        {
          ...
          Ke = Ke_[symidx(r0, regions[i_])];
          Kl = Kl_[symidx(r0, regions[i_])];
          dt0e_[i] += (Ke*(tempve-tempe)/wy/wy)*tempe/templ;
          d1 += (Ke*(tempe/templ-tempve/tempvl))/wy*(tempe-tempve)/wy;
          f1=1;
    ...
    if (iy+1<Ny){
      ...
      if (mm_!=0)
      {
        ...
        Kl = Kl_[symidx(r0, regions[i_])];
        dt0e_[i] += (Ke*(tempve-tempe)/wy/wy)*tempe/templ;
        d1 += (Ke*(tempve/tempvl-tempe/templ))/wy*(tempve-tempe)/wy;
        f1+=1;
        dt0l_[i] += (Kl*(tempvl-templ)/wy/wy);
      }
    }
    ...
    if (Nz != 1) {
        // bottom neighbor
           ...
           i_  = idx(ix, iy, iz-1);
             ...
             if (mm_!=0)
             {
                 ...
                 Ke = Ke_[symidx(r0, regions[i_])];
                 Kl = Kl_[symidx(r0, regions[i_])];
	               dt0e_[i] += (Ke*(tempve-tempe)/wz/wz)*tempe/templ;
                   d1 += (Ke*(tempe/templ-tempve/tempvl))/wz*(tempe-tempve)/wz;
                   f1=1;
        ...
        if (iz+1<Nz){
          ...
          if (mm_!=0)
          {
              ...
              Kl = Kl_[symidx(r0, regions[i_])];
	            dt0e_[i] += (Ke*(tempve-tempe)/wz/wz)*tempe/templ;
                d1 += (Ke*(tempve/tempvl-tempe/templ))/wz*(tempve-tempe)/wz;
                f1+=1;
	            dt0l_[i] += (Kl*(tempvl-templ)/wz/wz);
            }
         }
