...
addslonczewskitorque2LLB(float* __restrict__ tx, float* __restrict__ ty, float* __restrict__ tz,
    ...
    if (i < N) {
        ...
        // Remormalization of m and Ms->Ms(T)
        float  mm=sqrt(dot(m,m));
        //if (mm==0) mm=1;
        //float  mm2=1.0f/(mm*mm);
        //float  mm3=mm2/mm;
        m=1/mm*m;
        Ms=Ms*mm;
        ...
        float mxpxmFac = gilb * (A + alpha * B);  // mm3 To include M(T) dependence and normalization
        float pxmFac   = gilb * (B - alpha * A);  // mm2 To include M(T) dependence and normalization
